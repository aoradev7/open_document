...
class OpenDocumentPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    @RequiresApi(Build.VERSION_CODES.KITKAT)
    fun getPathDocument(path: String?, result: Result) {
    ...
    @RequiresApi(Build.VERSION_CODES.KITKAT)
    fun getName(url: String, result: Result) {
        result.success(name(url))
    }

    private fun getNameFolder(result: Result) {
        val name = nameFolder()
    ...
    @RequiresApi(Build.VERSION_CODES.KITKAT)
    fun checkDocument(filePath: String, result: Result) {
    ...
    private fun nameFolder(): String {
        return try {
            val appLabel = activity?.packageManager?.getApplicationInfo(context.packageName, 0)?.let {
                activity?.packageManager?.getApplicationLabel(it)
            }
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        flutterPluginBinding?.let {
            channel = MethodChannel(it.binaryMessenger, "open_document")
